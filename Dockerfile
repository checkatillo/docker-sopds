...
RUN rm -rf /var/cache/apk/* && \
     rm -rf /tmp/* && \
apk update && \
apk add --no-cache -U tzdata unzip build-base libxml2-dev libxslt-dev postgresql14-dev libffi-dev libc-dev jpeg-dev zlib-dev \
...
&& apk del tzdata unzip build-base libxml2-dev libxslt-dev postgresql14-dev libffi-dev libc-dev jpeg-dev zlib-dev \
&& rm -rf /root/.cache/ \
&& apk add --no-cache -U bash libxml2 libxslt libffi libjpeg zlib postgresql14 expect \
&& chmod +x /start.sh \
&& mkdir -p /sopds/tmp/ \
